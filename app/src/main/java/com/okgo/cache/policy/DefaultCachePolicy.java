...
public class DefaultCachePolicy<T> extends BaseCachePolicy<T> {
    ...
    public boolean onAnalysisResponse(final Call call, final okhttp3.Response response) {
        ...
        try {
            ...
            if (cacheEntity == null) {
                final Response<T> error = Response.error(true, body, call, response, CacheException.NON_AND_304(request.getCacheKey()));
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mCallback.onError(error);
            ...
            Response<T> error = Response.error(false, null, call, response, throwable);
            onError(error);
        }
        return true;
    }
    ...
    public Response<T> requestSync(CacheEntity<T> cacheEntity) {
        try {
            ...
            return Response.error(false, null, rawCall, null, throwable);
        }
        Response<T> response = requestNetworkSync();
        //HTTP cache protocol
        if (response.isSuccessful() && response.code() == 304) {
            if (cacheEntity == null) {
                response = Response.error(true, null, rawCall, response.getRawResponse(), CacheException.NON_AND_304(request.getCacheKey()));
            } else {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Response<T> error = Response.error(false, cacheEntity.getData(), rawCall, null, throwable);
                    mCallback.onError(error);
                    return;
                }
                requestNetworkAsync();
