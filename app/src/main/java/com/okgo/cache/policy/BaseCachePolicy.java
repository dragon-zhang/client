    ...
    protected Response<T> requestNetworkSync() {
        try {
            ...
            if (responseCode == 404 || responseCode >= 500) {
                return Response.error(false, body, rawCall, response, HttpException.NET_ERROR());
            }
        ...
        rawCall.enqueue(new okhttp3.Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                if (e instanceof SocketTimeoutException && currentRetryCount < request.getRetryCount()) {
                    ...
                    if (!call.isCanceled()) {
                        Response<T> error = Response.error(false, null, call, null, e);
                        onError(error);
                    }
                }
            }
            ...
            public void onResponse(Call call, okhttp3.Response response) throws IOException {
                ...
                try {
                    ...
                    //network error
                    if (responseCode == 404 || responseCode >= 500) {
                        Response<T> error = Response.error(false, body, call, response, HttpException.NET_ERROR());
                        onError(error);
                        return;
                    }
